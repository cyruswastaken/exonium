  ...
  const { mutate: signUpUser, isPending } = useMutation({
    ...
    onSuccess: (data: any) => {
      toast({
        ...
        description: "Welcome to Exonium.",
      });
      navigate(`/workspace/${data.data.user.currentWorkspace}`);
    },
    onError: (error: any) => {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md flex flex-col items-center gap-6">
        ...
        {!showEmailForm ? (
          <div className="w-full flex flex-col items-center gap-3 mt-2 mb-6">
            <button
              ...
              className="w-[400px] max-w-full rounded-md bg-[#5e6ad2] text-white py-3 font-medium transition duration-150 ease-in-out hover:brightness-105 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f79e6]/40 shadow-sm flex items-center justify-center gap-2"
            >
          ...
          <form onSubmit={handleEmailSignup} className="w-full flex flex-col items-center gap-4 mt-2 mb-6">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-[400px] max-w-full rounded-md bg-[#5e6ad2] text-white py-3 font-medium transition duration-150 ease-in-out hover:brightness-105 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6f79e6]/40 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <p className="text-xs text-[#b4bcd0] text-center max-w-sm mb-4">
          By creating an account, you agree to our <a href="#" className="font-semibold text-white underline">Terms of Service</a> and <a href="#" className="font-semibold text-white underline">Privacy Policy</a>.
        </p>
        <p className="text-xs text-[#b4bcd0] text-center">
          Already have an account?{' '}
          <Link to="/sign-in" className="font-semibold text-white underline">Sign in</Link>
        </p>
      </div>
    </motion.main>
  );
